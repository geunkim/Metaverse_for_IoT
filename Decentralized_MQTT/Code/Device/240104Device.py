...
first_connection = True  # 최초 연결 여부를 확인하는 변수

def on_connect(client, userdata, flags, rc):
    print("Connected with result code " + str(rc))

    # 최초 연결 시에만 DID 정보를 브로커로 전송
    if first_connection:
        payload = {"did": did_info}
        client.publish("initial_connection", json.dumps(payload))
...
async def main():
    ...
    try:
        for _ in range(5):  # 5번 전송
            ...
            # 서명과 데이터를 함께 publish
            # 최초 연결 시에만 특정 토픽으로 발행
            if first_connection:
                topic = "initial_connection_data"
            else:
                # 일반적인 데이터 토픽
                topic = f"{did_info}/myTopic1"

            payload = {"distance": distance, "signature": signature.decode(), "did":did_info}
